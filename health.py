#!/usr/bin/env python3
"""
Unified FastAPI Server - Health Checks + Telegram Webhook
Imports fyer.py as a module and runs the detector in a background thread.
"""
...
app = FastAPI(title="Fyers Detector Service", version="3.0.0")
...
WEBHOOK_URL = "https://fyers-volume-spike-detector.onrender.com/telegram-webhook"
...
def send_telegram(text):
    ...
    try:
        url = f"https://api.telegram.org/bot{fyer.TELEGRAM_BOT_TOKEN}/sendMessage"
        response = requests.post(url, data={
            "chat_id": fyer.TELEGRAM_CHAT_ID,
            "text": text,
            "parse_mode": "HTML"
        }, timeout=10)
...
def setup_telegram_webhook():
    ...
    url = f"https://api.telegram.org/bot{fyer.TELEGRAM_BOT_TOKEN}/setWebhook"
    payload = {
        "url": WEBHOOK_URL,
...
def remove_telegram_webhook():
    ...
    url = f"https://api.telegram.org/bot{fyer.TELEGRAM_BOT_TOKEN}/deleteWebhook"
    try:
...
async def telegram_webhook(request: Request):
    ...
    if chat_id != fyer.TELEGRAM_CHAT_ID:
        return {"ok": True}
...
async def root():
    return {
        "message": "Fyers Detector Service running",
        "status": "online",
...
def start_detector_thread():
    """Run the fyer supervisor loop in a background thread."""
...
if __name__ == "__main__":
    print("=" * 70)
    print("Fyers Detector Service - Unified Architecture")
    print("=" * 70)
