...
# GOOGLE_SHEETS_ID = os.getenv('GOOGLE_SHEETS_ID')  # Uncomment this line if you want to use environment variable
...
TELEGRAM_BOT_TOKEN = "8360146544:AAEObU8_9LoGTZk66PVSwcayD5Hw5fnHTgY"  # Your bot token
TELEGRAM_CHAT_ID = "5715256800"      # Your chat ID
# TELEGRAM_BOT_TOKEN = os.getenv('TELEGRAM_BOT_TOKEN')  # Uncomment this line if you want to use environment variable
# TELEGRAM_CHAT_ID = os.getenv('TELEGRAM_CHAT_ID')      # Uncomment this line if you want to use environment variable
...
class TelegramHandler:
    ...
    def extract_auth_code(self, message_text):
        ...
        try:
            # More flexible regex pattern to match auth_code
            # First try the specific pattern with state=None
    ...
    def wait_for_auth_code(self, timeout_seconds=TELEGRAM_AUTH_TIMEOUT):
        ...
        while time.time() - start_time < timeout_seconds:
            try:
                ...
                print("Retrying Telegram connection in 10 seconds...")
        ...
        try:
            ...
            for update in updates:
                if "message" in update and "text" in update["message"]:
                    ...
                    # Check if message contains "restart" command (case insensitive)
                    if message_text.lower() in ["restart", "restart!", "restart.", "restart bot", "restart system"]:
                        print(f"Restart command received from Telegram: '{message_text}'")
    ...
    def check_for_force_command(self):
        """Check for Force command in Telegram messages (case insensitive)"""
        try:
            updates = self.get_updates()
            for update in updates:
                if "message" in update and "text" in update["message"]:
                    message_text = update["message"]["text"].strip()
                    if message_text.lower() == "force":
                        print(f"Force command received from Telegram: '{message_text}'")
                        self.send_message("Force command received! Starting detector immediately, bypassing market hours.")
                        return True
            return False
        except Exception as e:
            print(f"Error checking for Force command: {e}")
            return False
...
class FyersAuthenticator:
    ...
    def load_saved_token(self):
        try:
            # First try to validate token from JSON file
 ...
 <b>Auto-retry:</b> Will send new link every 5 minutes until successful
    ...
    def update_code_file_with_new_token(self, token):
        ...
        try:
            ...
            print(f"Could not auto-update code file: {e}")
    ...
    def authenticate(self, max_retries=3):
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                print(f"Authorization URL generated (attempt {attempt + 1}): {response}")
                
                totp_code = self.generate_totp()
                print(f"Current TOTP code (if needed): {totp_code}")
                
                # Send auth URL and instructions to Telegram
                retry_text = f" (Retry {attempt + 1}/{max_retries})" if attempt > 0 else ""
                telegram_message = f"""
 <b>Fyers Authentication Required{retry_text}</b>
...
5. Send the redirect URL back to this bot
 ...
 <b>Timeout:</b> {TELEGRAM_AUTH_TIMEOUT} seconds
                ...
                if self.telegram.send_message(telegram_message):
                    ...
                    print("Failed to send to Telegram, falling back to manual input")
                
                # Wait for auth code from Telegram (with restart command checking)
                auth_code = self.telegram.wait_for_auth_code()
                
                # Check for restart command during auth code wait
                if self.telegram.check_for_restart_command():
                    print("Restart command received during authentication! Aborting current auth attempt...")
                    return False
                
                if not auth_code:
                    print("No auth code received from Telegram")
                    
                    # Check if we're in a deployment environment (no stdin available)
                    import sys
                    if not sys.stdin.isatty():
                        print("Running in deployment environment - cannot accept manual input")
                        print("Please send the auth code via Telegram or restart the service")
                ...
                print(f"Setting auth code: {auth_code[:20]}...")
                print(f"Auth code length: {len(auth_code)}")
                print(f"Auth code contains special chars: {bool(re.search(r'[%+]', auth_code))}")
                
                # Try URL decoding the auth code if it contains encoded characters
                import urllib.parse
                if '%' in auth_code or '+' in auth_code:
                    decoded_auth_code = urllib.parse.unquote(auth_code)
                    print(f"URL decoded auth code: {decoded_auth_code[:20]}...")
                    session.set_token(decoded_auth_code)
                else:
                    session.set_token(auth_code)
                
                print("Generating token from auth code...")
 ...
 Auto-updated in code
                    ...
                    # Auth code was rejected - this is where we retry
                    error_message = f"Authentication Failed (Attempt {attempt + 1}/{max_retries}):\n<code>{token_response}</code>"
                    if attempt < max_retries - 1:
                        error_message += f"\n\nRetrying automatically..."
                    self.telegram.send_message(error_message)
                    self.is_authenticated = False
                    print(f"Authentication failed (attempt {attempt + 1}): {token_response}")
                    
                    if attempt < max_retries - 1:
                        print("Auth code rejected, will retry with new code...")
                        continue  # Try next attempt
                    else:
                ...
                error_message = f"Authentication Error (Attempt {attempt + 1}/{max_retries}):\n<code>{str(e)}</code>"
                if attempt < max_retries - 1:
                    error_message += f"\n\nRetrying automatically..."
                self.telegram.send_message(error_message)
                self.is_authenticated = False
                ...
                if attempt < max_retries - 1:
                    print("Error occurred, will retry...")
                    continue  # Try next attempt
                else:
                    print("All authentication attempts failed!")
                    return False
        ...
        print("Authentication failed after all retry attempts!")
        return False
...
class GoogleSheetsManager:
    def __init__(self, detector=None):
        ...
        if not self.sheets_initialized:
            print("Google Sheets initialization failed. Data will not be saved to sheets.")
            ...
            with self.lock:
                ...
                if ltp_color_format:
                    try:
                        ...
                        color_indicator = "Green" if ltp_change > 0 else "🔴" if ltp_change < 0 else "⚪"
                        print(f"Added to Google Sheets: {symbol} ({sector}) {color_indicator} - Rs.{trade_value/10000000:.2f} crore (LTP: Rs.{ltp:,.2f}, Change: {ltp_change:+.2f})")
                    except Exception as e:
                        print(f"Added to sheets but color formatting failed: {e}")
...
class VolumeSpikeDetector:
    def __init__(self):
        ...
        self.stop_event = None  # Will be set by the supervisor
    ...
    def initialize(self):
        ...
        print(f"Individual trade threshold: Rs.{INDIVIDUAL_TRADE_THRESHOLD:,} ({INDIVIDUAL_TRADE_THRESHOLD/10000000:.1f} crore)")
        print(f"Google Sheet: https://docs.google.com/spreadsheets/d/{GOOGLE_SHEETS_ID}")
        print(f"Sector mapping: {len(SECTOR_MAPPING)} symbols mapped")
...
• Threshold: Rs.{INDIVIDUAL_TRADE_THRESHOLD:,} ({INDIVIDUAL_TRADE_THRESHOLD/10000000:.1f} crore)
...
🕐 <b>Started:</b> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
            ...
            print(f"Could not send startup message to Telegram: {telegram_error}")
...
Status: Maximum retries ({self.max_websocket_retries}) reached
Action: Triggering re-authentication
...
Attempt: {self.websocket_retry_count + 1}/{self.max_websocket_retries}
...
Sector: {sector}
LTP: ₹{ltp:,.2f} ({ltp_change:+.2f})
Volume Spike: {volume_spike:,.0f} shares
Trade Value: ₹{individual_trade_value:,.0f} ({individual_trade_value/10000000:.2f} crore)
...
Time: {datetime.now().strftime('%H:%M:%S')}
            ...
            if self.total_ticks % 2000 == 0:
                ...
                if self.total_ticks % 10000 == 0:
                    stats_message = f"""
Volume Spike Detector Statistics

Runtime: {runtime/60:.1f} minutes
Ticks Processed: {self.total_ticks:,}
Large Trades Detected: {self.individual_trades_detected}
Symbols Tracked: {tracked_symbols}
...
Trades Detected: {self.individual_trades_detected}
Time: {datetime.now().strftime('%H:%M:%S')}
"""
...
Time: {datetime.now().strftime('%H:%M:%S')}
                    ...
                    print("Waiting 5 seconds before attempting to restart...")
                    time.sleep(5)
...
Error: {str(e)}
Retries: {self.websocket_retry_count}/{self.max_websocket_retries}
Action: Triggering re-authentication
Time: {datetime.now().strftime('%H:%M:%S')}
                    """
                    self.authenticator.telegram.send_message(reauth_message)
...
Large Trades Detected: {self.individual_trades_detected}
Google Sheet: <a href="https://docs.google.com/spreadsheets/d/{GOOGLE_SHEETS_ID}">View Data</a>
...
"""
                    ...
                    print("Retrying initialization in 60 seconds...")
                    time.sleep(60)
                    self.run()  # Recursive restart
        except Exception as e:
            print(f"Critical error in run method: {e}")
            print("Restarting entire application in 60 seconds...")
            import traceback
            traceback.print_exc()
            time.sleep(60)
            self.run()  # Recursive restart
...
print("Comprehensive sector mapping for NSE stocks")
...
print("Google Sheets credentials loaded from environment variables")

print(f"Sector Mapping Statistics:")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if telegram.check_for_force_command():
            print("Force command detected. Starting detector immediately, bypassing market hours.")
            detector = VolumeSpikeDetector()
            detector.run()
        else:
