...
# =============================================================================
# FLASK APP FOR HEALTH ENDPOINT (Render Keep-Alive)
# =============================================================================
...
def root():
    """Root endpoint"""
    return jsonify({
        'message': 'Fyers Volume Spike Detector is running',
        'timestamp': datetime.now().isoformat()
    })
...
try:
    ...
    print("⚠️  Fyers access token JSON file not found, using environment variables")
except Exception as e:
    # Fallback to environment variables on any error
    FYERS_ACCESS_TOKEN = os.getenv('FYERS_ACCESS_TOKEN', '')
    FYERS_TOKEN_TIMESTAMP = float(os.getenv('FYERS_TOKEN_TIMESTAMP', '0'))
    FYERS_TOKEN_CREATED_AT = os.getenv('FYERS_TOKEN_CREATED_AT', '')
    print(f"⚠️  Error loading Fyers token from JSON: {e}, using environment variables")
...
🔄 Auto-updated in code
                    ...
                    error_message = f"❌ <b>Authentication Failed (Attempt {attempt + 1}/{max_retries}):</b>\n<code>{token_response}</code>"
                ...
                error_message = f"❌ <b>Authentication Error (Attempt {attempt + 1}/{max_retries}):</b>\n<code>{str(e)}</code>"
                if attempt < max_retries - 1:
                    error_message += f"\n\n🔄 <b>Retrying automatically...</b>"
                self.telegram.send_message(error_message)
                self.is_authenticated = False
...
🕐 <b>Started:</b> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
            ...
            print(f"⚠️  Could not send startup message to Telegram: {telegram_error}")
